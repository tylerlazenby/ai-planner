...
const generatePlanForDate = (dateStr: string) => {
...
export default function PlanDetailPage({ params }: { params: { date: string } }) {
    ...
    useEffect(() => {
        ...
        const planData = generatePlanForDate(params.date)
        setPlan(planData)
        setLoading(false)
    }, [params.date])
    ...
    const toggleTaskCompletion = (taskId: string) => {
        ...
        setPlan({
            ...plan,
            tasks: plan.tasks.map((task: any) => (task.id === taskId ? { ...task, completed: !task.completed } : task)),
        })
    }

    // Check if a task falls within a time slot
    const isTaskInTimeSlot = (task: any, slotTime: Date) => {
    ...
    const getTasksForTimeSlot = (slotTime: Date) => {
        if (!plan) return []
        return plan.tasks.filter((task: any) => isTaskInTimeSlot(task, slotTime))
    }
    ...
    const isFirstSlotForTask = (task: any, slotTime: Date) => {
        const taskStart = parse(task.startTime, "HH:mm", new Date())
